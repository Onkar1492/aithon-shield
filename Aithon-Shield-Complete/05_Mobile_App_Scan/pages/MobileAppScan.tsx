...
export default function MobileAppScan() {
  ...
  const { data: selectedScan } = useQuery<MobileAppScan>({
    ...
    refetchInterval: (query) => {
      const data = query.state.data;
      if (data?.scanStatus === "scanning" || data?.uploadStatus === "pending" || (data?.uploadProgress && data.uploadProgress !== 'idle')) {
        return 1000;
      }
      return false;
    },
            ...
            <CardContent className="space-y-4">
              ...
              {selectedScan.scanStatus === "scanning" && (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground" data-testid={`text-scanning-${selectedScan.id}`}>Analyzing app security...</p>
                </div>
              )}
