...
import { Globe, Shield, Upload, CheckCircle, AlertTriangle, Server, Info, Copy, Lightbulb, MapPin, Hand, Zap, Pencil, Trash2, XCircle, Loader2 } from "lucide-react";
import { SiVercel, SiNetlify, SiHeroku } from "react-icons/si";
...
import { EditWebAppScanDialog } from "@/components/EditWebAppScanDialog";
...
export default function WebAppScan() {
  const { toast } = useToast();
  const [, routeParams] = useRoute("/web-scans/:id");
  ...
  const { data: scans = [], isLoading } = useQuery<WebAppScan[]>({
    queryKey: ["/api/web-scans"],
    refetchInterval: (query) => {
      const data = query.state.data;
      // Auto-refresh if any scan is scanning
      if (data?.some(scan => scan.scanStatus === "scanning" || scan.scanStatus === "pending")) {
        return 2000;
      }
      return false;
    },
  ...
  const { data: selectedScan } = useQuery<WebAppScan>({
    queryKey: ["/api/web-scans", selectedScanId],
    enabled: !!selectedScanId,
    refetchInterval: (query) => {
      const data = query.state.data;
      if (data?.scanStatus === "scanning" || data?.uploadStatus === "pending" || (data?.uploadProgress && data.uploadProgress !== 'idle') || data?.testStatus === "running") {
        return 1000;
      }
      return false;
    },
  ...
  const startScanMutation = useMutation({
    mutationFn: async (scanId: string) => {
      const res = await apiRequest("POST", `/api/web-scans/${scanId}/scan`);
  ...
  const uploadMutation = useMutation({
    mutationFn: async ({ scanId, withFixes, runTests }: { scanId: string; withFixes: boolean; runTests?: boolean }) => {
      const endpoint = runTests ? `/api/web-scans/${scanId}/upload-and-test` : `/api/web-scans/${scanId}/upload`;
    ...
    onSuccess: (_, variables) => {
      ...
      toast({
        title: variables.runTests ? "Upload & Testing in progress" : "Upload in progress",
        description: variables.runTests 
          ? "Web app is being uploaded and comprehensive tests will run automatically"
          : `Web app ${variables.withFixes ? "with fixes" : "without fixes"} is being uploaded to ${selectedScan?.hostingPlatform}`,
      });
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (scanId: string) => {
      const res = await apiRequest("DELETE", `/api/web-scans/${scanId}`);
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/web-scans"] });
  ...
  const cancelScanMutation = useMutation({
    mutationFn: async (scanId: string) => {
      const response = await fetch(`/api/web-scans/${scanId}/cancel`, {
        method: 'PATCH',
        credentials: 'include',
      });
  ...
  const handleUploadClick = async (withFixes: boolean) => {
    // Ensure a scan is selected (defensive check matching MVP/Mobile pattern)
    if (!selectedScanId) {
      toast({
    ...
    // If uploading without fixes, show warning dialog first (matching MVP/Mobile workflow)
    setShowUploadWithoutFixesWarning(true);
  };

  const confirmUpload = () => {
    if (selectedScan) {
      // Close dialog immediately to show upload is in progress
      setShowUploadDialog(false);
      
      // Show immediate feedback toast
      toast({
        title: "Upload starting...",
        description: `Preparing to upload web app ${uploadWithFixes ? "with fixes" : "without fixes"} to ${selectedScan.hostingPlatform || "hosting platform"}`,
      });
  ...
  const handleProceedWithUpload = (runTests?: boolean) => {
    if (!selectedScanId || !selectedScan?.hostingPlatform) {
      toast({
        title: "Missing information",
        description: "Cannot upload without scan ID and hosting platform",
        variant: "destructive",
      });
      return;
    }
    ...
    upload({
      ...
      destination: selectedScan.hostingPlatform,
      withFixes: true,
      runTests,
    });
  ...
  const getHostingPlatformIcon = (platform: string) => {
  ...
  return (
    <div className="container mx-auto p-6 max-w-7xl space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight" data-testid="heading-web-scan">Web App Security Scan</h1>
        <p className="text-muted-foreground mt-2" data-testid="text-web-scan-description">
          Scan your live web applications for vulnerabilities and upload fixes back to hosting platform
        </p>
      </div>

      <ChooseWorkflowSection 
        title="Start Web App Scan"
        description="Choose your workflow to scan web applications from live URLs or existing deployments"
        defaultAppType="web"
        hideTabs={true}
      />
        ...
        {selectedScan && (
          <Card data-testid={`card-scan-details-${selectedScan.id}`}>
            <CardHeader>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <CardDescription className="mt-1 break-all" data-testid={`text-scan-url-${selectedScan.id}`}>
                    {selectedScan.appUrl}
                  </CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2 p-3 bg-muted rounded-lg" data-testid={`info-hosting-platform-${selectedScan.id}`}>
                {(() => {
                  const Icon = getHostingPlatformIcon(selectedScan.hostingPlatform);
                  return <Icon className="h-4 w-4 text-muted-foreground" />;
                })()}
                <p className="text-sm" data-testid={`text-hosting-platform-${selectedScan.id}`}>Hosted on {selectedScan.hostingPlatform}</p>
              </div>

              {selectedScan.scanStatus === "scanning" && (
                <div className="space-y-2">
                  <Progress value={65} className="h-2" data-testid={`progress-scan-${selectedScan.id}`} />
                  <p className="text-sm text-muted-foreground" data-testid={`text-scanning-${selectedScan.id}`}>Analyzing web app security...</p>
                </div>
              )}
                  ...
                  {selectedScan.uploadStatus === "uploaded" && (
                    <>
                      <div className="flex items-center gap-2 p-4 bg-green-500/10 rounded-lg border border-green-500/20" data-testid={`alert-uploaded-${selectedScan.id}`}>
                        ...
                        <p className="text-sm font-medium text-green-500">Successfully uploaded to {selectedScan.hostingPlatform}</p>
                      </div>
                      <div className="space-y-2 border-t pt-4">
                        <p className="text-sm font-medium" data-testid={`label-reupload-${selectedScan.id}`}>Re-upload to Hosting Platform</p>
      ...
      <Card data-testid="card-all-scans">
        <CardHeader>
          <CardTitle data-testid="heading-all-scans">All Web App Scans</CardTitle>
          <CardDescription data-testid="text-all-scans-description">
            View and manage your web application security scans
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground" data-testid="text-no-scans">
              No scans yet. Register a web app to get started.
            </div>
          ) : (
            <div className="space-y-3">
              {scans.map((scan) => {
                const Icon = getHostingPlatformIcon(scan.hostingPlatform);
                return (
                  <div
                    key={scan.id}
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <Icon className={`h-5 w-5 flex-shrink-0 ${selectedScanId === scan.id ? "text-primary" : "text-muted-foreground"}`} />
                      <div className="min-w-0 flex-1">
                        <p className={`font-medium ${selectedScanId === scan.id ? "text-foreground" : ""}`} data-testid={`text-scan-name-${scan.id}`}>{scan.appName}</p>
                        <p className="text-sm text-muted-foreground truncate" data-testid={`text-scan-url-${scan.id}`}>
                          {scan.appUrl}
                        </p>
                      </div>
                    </div>
      ...
      <UploadWithFixesOptionsDialog
        ...
        scanType="web"
        scanId={selectedScanId || ""}
        destination={selectedScan?.hostingPlatform || "hosting platform"}
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2" data-testid="heading-upload-in-progress">
                  ...
                  Uploading to {selectedScan?.hostingPlatform}...
                </DialogTitle>
                <DialogDescription data-testid="text-upload-status">
                  Your web app {uploadWithFixes ? "with security fixes" : ""} is being uploaded. This may take a moment...
                </DialogDescription>
              </DialogHeader>
            ...
            <>
              <DialogHeader>
                <DialogTitle data-testid="heading-dialog-upload">
                  Confirm Upload to {selectedScan?.hostingPlatform}
                </DialogTitle>
                <DialogDescription data-testid="text-dialog-upload-description">
                  {uploadWithFixes 
                    ? `This will upload your web app WITH security fixes applied back to ${selectedScan?.hostingPlatform}. The fixes will be automatically deployed.`
                    : `This will upload your web app WITHOUT any fixes back to ${selectedScan?.hostingPlatform}. No changes will be made to your application.`
                  }
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
