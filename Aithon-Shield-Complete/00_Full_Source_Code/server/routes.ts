...
// ${issue.description}`;
  ...
  app.patch("/api/user/profile", requireAuth, async (req: any, res) => {
    try {
      ...
      // If email is being updated, check if it already exists (case-insensitive vs current row)
      if (validatedData.email) {
        const next = validatedData.email.toLowerCase();
        const current = (req.user.email as string).toLowerCase();
        if (next !== current) {
          const existingEmail = await storage.getUserByEmail(validatedData.email);
  ...
  app.patch("/api/mobile-scans/:id/cancel", requireAuth, async (req: any, res) => {
    try {
      ...
      // Set cancellation flag and update status
  ...
  app.post("/api/mobile-scans/:id/scan", requireAuth, async (req: any, res) => {
    try {
      ...
      const appUrl = (req.body.appUrl as string) || 
        (scan.platform === 'ios' 
          ? `https://apps.apple.com/app/id${scan.appId}` 
          : `https://play.google.com/store/apps/details?id=${scan.appId}`);
      
      const mobileValidation = validateMobileAppScan(scan.platform, scan.appId, appUrl);
      if (!mobileValidation.valid) {
        await storage.updateMobileAppScan(req.params.id, req.user.id, {
          scanStatus: 'failed',
          scanError: mobileValidation.error || 'Invalid mobile app configuration',
        });
        return res.status(400).json({ message: mobileValidation.error || 'Invalid mobile app configuration' });
      }

      // Update scan status to scanning and initialize progress tracking
      await storage.updateMobileAppScan(req.params.id, req.user.id, {
        scanStatus: 'scanning',
        scanProgress: 0,
        scanStage: 'Initializing mobile app scan...',
        scanError: null,
      ...
      const progressCallback = async (progress: number, stage: string) => {
        ...
        const currentScan = await storage.getMobileAppScan(req.params.id, req.user.id);
      ...
      scanMobileApp(
        ...
        console.error('Mobile scan error:', formatErrorForLogging(error));
        ...
        // Log error
        console.error(`Mobile scan ${isCancellation ? 'cancelled' : 'failed'}:`, scanError.userMessage);
      });
  ...
  app.post("/api/mvp-scans/:id/scan", requireAuth, async (req: any, res) => {
    try {
      ...
      // Validate scan inputs before starting
      const mvpValidation = validateMvpRepositoryUrl(scan.repositoryUrl);
      if (!mvpValidation.valid) {
        await storage.updateMvpCodeScan(req.params.id, req.user.id, {
          scanStatus: 'failed',
          scanError: mvpValidation.error || 'Invalid repository URL',
        });
        return res.status(400).json({ message: mvpValidation.error || 'Invalid repository URL' });
      }
  ...
  app.post("/api/web-scans/:id/scan", requireAuth, async (req: any, res) => {
    try {
      ...
      // Validate scan inputs before starting
      const webUrlValidation = validateWebAppUrl(scan.appUrl);
      if (!webUrlValidation.valid) {
        await storage.updateWebAppScan(req.params.id, req.user.id, {
          scanStatus: 'failed',
          scanError: webUrlValidation.error || 'Invalid application URL',
        });
        return res.status(400).json({ message: webUrlValidation.error || 'Invalid application URL' });
      }

      // Validate authentication configuration if required
      const authValidation = validateWebAppAuth(
        scan.authRequired || false,
        scan.authType,
        scan.authUsername,
        scan.authPassword,
        scan.authApiKey,
      ...
      if (!authValidation.valid) {
        await storage.updateWebAppScan(req.params.id, req.user.id, {
          scanStatus: 'failed',
          scanError: authValidation.error || 'Invalid authentication configuration',
        });
        return res.status(400).json({ message: authValidation.error || 'Invalid authentication configuration' });
      }
      ...
      await storage.updateWebAppScan(req.params.id, req.user.id, {
        scanStatus: 'scanning',
        scanProgress: 0,
        scanStage: 'Initializing web scan...',
        scanError: null,
        cancellationRequested: false,
      });
      ...
      const progressCallback = async (progress: number, stage: string) => {
        // Check if cancellation was requested
        const currentScan = await storage.getWebAppScan(req.params.id, req.user.id);
        if (currentScan?.cancellationRequested) {
          throw new Error('Scan cancellation requested by user');
        }
      ...
      scanWebApp(
        scan.appUrl,
        {
          authenticationType: scan.authType || undefined,
          username: scan.authUsername || undefined,
        ...
        // Log detailed error for debugging
        console.error('Web scan error:', formatErrorForLogging(error));
        ...
        // Log error
        console.error(`Web scan ${isCancellation ? 'cancelled' : 'failed'}:`, scanError.userMessage);
      });
...
<body>
  ...
  app.get("/api/linter-scans/:id/fixed-code", requireAuth, async (req: any, res) => {
    try {
      ...
      const extractFirstAfterBlock = (suggestion: string): string | null => {
