...
export default function MvpCodeScan() {
  const { toast } = useToast();
  const [, routeParams] = useRoute("/mvp-scans/:id");
  const [selectedScanId, setSelectedScanId] = useState<string | null>(routeParams?.id || null);
  const [showUploadDialog, setShowUploadDialog] = useState(false);
  const [uploadWithFixes, setUploadWithFixes] = useState(true);
  const [showAppStoreDialog, setShowAppStoreDialog] = useState(false);
  const [targetAppStore, setTargetAppStore] = useState<string | null>(null);
  const [appStoreBundleId, setAppStoreBundleId] = useState("");
  const [appStoreUploadWithFixes, setAppStoreUploadWithFixes] = useState(true);
  ...
  // NEW: Shared upload dialog states for Source Platform
  ...
  // NEW: Shared upload dialog states for App Store
  const [showAppStoreUploadWithoutFixesWarning, setShowAppStoreUploadWithoutFixesWarning] = useState(false);
  const [showAppStoreUploadWithFixesOptions, setShowAppStoreUploadWithFixesOptions] = useState(false);
  ...
  const { data: scans = [], isLoading } = useQuery<MvpCodeScan[]>({
    ...
    refetchInterval: (query) => {
      ...
      // Auto-refresh if any scan is scanning or cancelling
  ...
  const { data: selectedScan } = useQuery<MvpCodeScan>({
    ...
    refetchInterval: (query) => {
      const data = query.state.data;
      if (data?.scanStatus === "scanning" || data?.scanStatus === "cancelling" || data?.uploadStatus === "pending" || (data?.uploadProgress && data.uploadProgress !== 'idle') || (data?.appStoreUploadStatus === "pending") || (data?.appStoreUploadProgress && data.appStoreUploadProgress !== 'idle') || data?.testStatus === "running") {
        return 1000;
      }
      return false;
    },
  ...
  const { data: qrCodeData } = useQuery<{ qrCode: string; previewUrl: string }>({
    queryKey: ["/api/mvp-scans", selectedScanId, "qrcode"],
    enabled: !!selectedScanId && selectedScan?.scanStatus === "completed" && !!selectedScan?.previewUrl,
  });

  const startScanMutation = useMutation({
    mutationFn: async (scanId: string) => {
      const res = await apiRequest("POST", `/api/mvp-scans/${scanId}/scan`);
  ...
  const uploadMutation = useMutation({
    mutationFn: async ({ scanId, withFixes, runTests }: { scanId: string; withFixes: boolean; runTests?: boolean }) => {
      const endpoint = runTests ? `/api/mvp-scans/${scanId}/upload-and-test` : `/api/mvp-scans/${scanId}/upload`;
    ...
    onSuccess: (_, variables) => {
      ...
      toast({
        ...
        description: variables.runTests 
          ? "Code is being uploaded and comprehensive tests will run automatically"
          : `Code ${variables.withFixes ? "with fixes" : "without fixes"} is being uploaded to ${selectedScan?.platform}`,
      });
    },
  });

  const appStoreUploadMutation = useMutation({
    mutationFn: async ({ scanId, withFixes, targetAppStore, appStoreBundleId }: { scanId: string; withFixes: boolean; targetAppStore: string; appStoreBundleId: string }) => {
      const res = await apiRequest("POST", `/api/mvp-scans/${scanId}/upload-to-store`, { 
        withFixes,
        targetAppStore,
        appStoreBundleId
      });
    ...
    onSuccess: () => {
      ...
      setTargetAppStore(null);
      setAppStoreBundleId("");
      toast({
        title: "App Store upload initiated",
        description: `App ${appStoreUploadWithFixes ? "with fixes" : "without fixes"} is being uploaded to ${targetAppStore === 'ios' ? 'iOS App Store' : 'Android Play Store'}`,
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (scanId: string) => {
      const res = await apiRequest("DELETE", `/api/mvp-scans/${scanId}`);
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/mvp-scans"] });
      ...
      toast({
        title: "Scan deleted",
        description: "The scan and all associated findings have been deleted",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const cancelScanMutation = useMutation({
    mutationFn: async (scanId: string) => {
      const response = await fetch(`/api/mvp-scans/${scanId}/cancel`, {
        method: 'PATCH',
        credentials: 'include',
      });
  ...
  const confirmUpload = () => {
    if (selectedScan) {
      uploadMutation.mutate({ scanId: selectedScan.id, withFixes: uploadWithFixes });
    }
  };
  ...
  const handleProceedWithUpload = (runTests?: boolean) => {
    if (!selectedScanId || !selectedScan?.platform) {
      toast({
        title: "Missing information",
        description: "Cannot upload without scan ID and destination platform",
        variant: "destructive",
      });
      return;
    }
    ...
    upload({
      ...
      destination: selectedScan.platform,
      withFixes: true,
      runTests,
    });
  ...
  const handleAppStoreUploadClick = (withFixes: boolean) => {
    setAppStoreUploadWithFixes(withFixes);
    setShowAppStoreDialog(true);
  };

  const confirmAppStoreUpload = () => {
    if (selectedScan && targetAppStore && appStoreBundleId) {
      appStoreUploadMutation.mutate({ 
        scanId: selectedScan.id, 
        withFixes: appStoreUploadWithFixes,
        targetAppStore,
        appStoreBundleId
      });
    } else {
      toast({
        title: "Missing information",
        description: "Please select app store and provide Bundle ID/Package Name",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 max-w-7xl space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight" data-testid="heading-mvp-scan">MVP Code Security Scan</h1>
        <p className="text-muted-foreground mt-2" data-testid="text-mvp-scan-description">
          Scan your MVP code from various platforms, identify vulnerabilities, and upload fixes back to source
        </p>
      </div>

      <ChooseWorkflowSection 
        title="Start MVP Code Scan"
        description="Choose your workflow to scan MVP code from repository or existing deployment"
        defaultAppType="mvp"
        hideTabs={true}
      />
            ...
            <CardHeader>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <CardDescription className="mt-1" data-testid={`text-scan-details-${selectedScan.id}`}>
                    {selectedScan.platform} • {selectedScan.branch}
                  </CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              ...
              {selectedScan.scanStatus === "completed" && (
                <div className="space-y-4">
                  ...
                  {qrCodeData && (
                    <div className="space-y-3 p-4 bg-accent/30 rounded-lg border" data-testid={`section-qr-preview-${selectedScan.id}`}>
                      <div className="flex items-center gap-2">
                        <QrCode className="h-5 w-5 text-primary" />
                        <p className="text-sm font-medium">Preview with Fixes</p>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        Scan this QR code with your phone to preview how your app will look with security fixes applied before uploading.
                      </p>
                      <div className="flex justify-center p-4 bg-white rounded-lg">
                        <img 
                          ...
                          alt="Preview QR Code" 
                          className="w-48 h-48"
                          data-testid={`img-qr-code-${selectedScan.id}`}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {selectedScan.uploadStatus === "none" && (
                    <div className="space-y-2">
                      <p className="text-sm font-medium" data-testid={`label-upload-options-${selectedScan.id}`}>Upload to Source Platform</p>
                  ...
                  {/* App Store Upload Section */}
                  {(!selectedScan.appStoreUploadStatus || selectedScan.appStoreUploadStatus === "none") && (
                    <div className="space-y-2 border-t pt-4">
                      <p className="text-sm font-medium" data-testid={`label-app-store-upload-${selectedScan.id}`}>Upload to App Store</p>
                      <p className="text-sm text-muted-foreground">
                        Deploy your MVP directly to iOS App Store or Android Play Store
                      </p>
                      <div className="grid grid-cols-2 gap-2">
                        <Button
                          variant="default"
                          className="gap-2"
                          onClick={() => handleAppStoreUploadClick(true)}
                          disabled={appStoreUploadMutation.isPending}
                          data-testid={`button-app-store-review-fix-issues-${selectedScan.id}`}
                        >
                          <CheckCircle className="h-4 w-4" />
                          Review & Fix Issues
                        </Button>
                        <Button
                          variant="outline"
                          className="gap-2"
                          onClick={() => handleAppStoreUploadClick(false)}
                          disabled={appStoreUploadMutation.isPending}
                          data-testid={`button-app-store-upload-without-fixes-${selectedScan.id}`}
                        >
                          <AlertTriangle className="h-4 w-4" />
                          Upload Without Fixes
                        </Button>
                    ...
                    <>
                      <div className="flex items-center gap-2 p-4 bg-green-500/10 rounded-lg border border-green-500/20" data-testid={`alert-uploaded-${selectedScan.id}`}>
                        ...
                        <p className="text-sm font-medium text-green-500">
                          Successfully uploaded to {selectedScan.platform}
                          {selectedScan.fixesApplied && selectedScan.uploadPreference === 'fix-and-upload' && " with security fixes"}
                        </p>
                      </div>
                      {/* Only show re-upload option if fixes weren't applied during initial upload */}
                      {(!selectedScan.fixesApplied || selectedScan.uploadPreference !== 'fix-and-upload') && (
                        <div className="space-y-2 border-t pt-4">
                          <p className="text-sm font-medium" data-testid={`label-reupload-${selectedScan.id}`}>Re-upload to Source Platform</p>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <Button
                              ...
                              onClick={() => handleUploadClick(true)}
                              disabled={uploadMutation.isPending}
                              data-testid={`button-reupload-with-fixes-${selectedScan.id}`}
                            >
                              <CheckCircle className="h-4 w-4" />
                  ...
                  {/* App Store Upload Progress */}
                  {selectedScan.appStoreUploadStatus === "pending" && selectedScan.appStoreUploadProgress && (
                    <div className="border-t pt-4">
                      <UploadProgress 
                        uploadProgress={selectedScan.appStoreUploadProgress} 
                        destination={selectedScan.targetAppStore === 'ios' ? 'iOS App Store' : 'Android Play Store'}
                      />
                    </div>
                  )}

                  {selectedScan.appStoreUploadStatus === "uploaded" && (
                    <div className="flex items-center gap-2 p-4 bg-green-500/10 rounded-lg border border-green-500/20 border-t pt-4" data-testid={`alert-app-store-uploaded-${selectedScan.id}`}>
                      <CheckCircle className="h-5 w-5 text-green-500" />
                      <p className="text-sm font-medium text-green-500">
                        Successfully uploaded to {selectedScan.targetAppStore === 'ios' ? 'iOS App Store' : 'Android Play Store'}
                      </p>
                    </div>
                  )}
                </div>
      ...
      <Card data-testid="card-all-scans">
        <CardHeader>
          <CardTitle data-testid="heading-all-scans">All MVP Code Scans</CardTitle>
          <CardDescription data-testid="text-all-scans-description">
            View and manage your MVP code security scans
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground" data-testid="text-no-scans">
              No scans yet. Register a project to get started.
            </div>
          ) : (
              ...
              {scans.map((scan) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <Code className={`h-5 w-5 ${selectedScanId === scan.id ? "text-primary" : "text-muted-foreground"}`} />
                    <div>
                      <p className={`font-medium ${selectedScanId === scan.id ? "text-foreground" : ""}`} data-testid={`text-scan-name-${scan.id}`}>{scan.projectName}</p>
                      <p className="text-sm text-muted-foreground" data-testid={`text-scan-platform-${scan.id}`}>
                        {scan.platform} • {scan.branch}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
      ...
      <UploadWithFixesOptionsDialog
        ...
        scanType="mvp"
        scanId={selectedScanId || ""}
        destination={selectedScan?.platform || "deployment platform"}
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2" data-testid="heading-upload-in-progress">
                  ...
                  Uploading to {selectedScan?.platform}...
                </DialogTitle>
                <DialogDescription data-testid="text-upload-status">
                  Your code {uploadWithFixes ? "with security fixes" : ""} is being uploaded. This may take a moment...
                </DialogDescription>
              </DialogHeader>
            ...
            <>
              <DialogHeader>
                <DialogTitle data-testid="heading-dialog-upload">
                  Confirm Upload to {selectedScan?.platform}
                </DialogTitle>
                <DialogDescription data-testid="text-dialog-upload-description">
                  {uploadWithFixes 
                    ? `This will upload your code WITH security fixes applied back to ${selectedScan?.platform}. The fixes will be automatically committed to the ${selectedScan?.branch} branch.`
                    : `This will upload your code WITHOUT any fixes back to ${selectedScan?.platform}. No changes will be made to your code.`
                  }
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
      ...
      {/* App Store Upload Dialog */}
      <Dialog open={showAppStoreDialog} onOpenChange={setShowAppStoreDialog}>
        <DialogContent data-testid="dialog-app-store-upload">
          <DialogHeader>
            <DialogTitle data-testid="heading-dialog-app-store-upload">
              Upload to App Store
            </DialogTitle>
            <DialogDescription data-testid="text-dialog-app-store-description">
              {appStoreUploadWithFixes 
                ? `Deploy your MVP WITH security fixes applied to the app store. The app will be packaged and submitted for review.`
                : `Deploy your MVP WITHOUT any fixes to the app store. The original version will be submitted for review.`
              }
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="target-store" data-testid="label-target-store">Target Store</Label>
              <Select value={targetAppStore || ""} onValueChange={setTargetAppStore}>
                <SelectTrigger id="target-store" data-testid="select-target-store">
                  <SelectValue placeholder="Select app store" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="ios" data-testid="option-ios">
                    <div className="flex items-center gap-2">
                      <Apple className="h-4 w-4" />
                      iOS App Store
                    </div>
                  </SelectItem>
                  <SelectItem value="android" data-testid="option-android">
                    <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="bundle-id" data-testid="label-bundle-id">
                {targetAppStore === 'ios' ? 'Bundle ID' : 'Package Name'}
              </Label>
              <Input
                id="bundle-id"
                value={appStoreBundleId}
                onChange={(e) => setAppStoreBundleId(e.target.value)}
                placeholder={targetAppStore === 'ios' ? 'com.company.appname' : 'com.company.appname'}
                data-testid="input-bundle-id"
              />
              <p className="text-sm text-muted-foreground">
                {targetAppStore === 'ios' 
                  ? 'Enter your iOS app Bundle Identifier' 
          ...
          <DialogFooter>
            <Button 
              ...
              data-testid="button-cancel-app-store-upload"
            >
              Cancel
            </Button>
            <Button 
              onClick={confirmAppStoreUpload}
              disabled={!targetAppStore || !appStoreBundleId}
              data-testid="button-confirm-app-store-upload"
            >
              <Upload className="h-4 w-4 mr-2" />
              Confirm Upload
            </Button>
        ...
        <AlertDialogContent data-testid="dialog-delete-confirmation">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription data-testid="text-delete-description">
              Are you sure you want to delete "{selectedScan?.projectName}"? This will permanently delete the scan and all associated security findings. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
