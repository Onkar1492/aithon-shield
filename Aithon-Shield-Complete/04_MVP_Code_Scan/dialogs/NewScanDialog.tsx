...
export function NewScanDialog({ open: controlledOpen, onOpenChange, preselectedType }: NewScanDialogProps = {}) {
  ...
  const onMvpSubmit = async (data: MvpScanFormData) => {
    try {
      const scanData: InsertMvpCodeScan = {
      ...
      const result: any = await mvpMutation.mutateAsync(scanData);
  ...
  const onMobileSubmit = async (data: MobileScanFormData) => {
    try {
      const scanData: InsertMobileAppScan = {
      ...
      const result: any = await mobileMutation.mutateAsync(scanData);
  ...
  const onWebSubmit = async (data: WebScanFormData) => {
    try {
      const scanData: InsertWebAppScan = {
        appUrl: data.url,
        appName: data.appName,
      ...
      const result: any = await webMutation.mutateAsync(scanData);
